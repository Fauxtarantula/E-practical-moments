...
/*//Step 1
  ...
  do {
    //Step 4
    Serial.println(F("\nStep 4: Set up the apparatus using the instruction sheet provided\n--> Use d = 0.28m for your first set of readings\n"));
    while (Serial.available() == 0);
    Serial.read();

    //Step 5
    Serial.print(F("Step 5: Check that the beam is horizontal using the accelerometer readings\nIs the beam horizontal?   (Type in 'YES' when the beam is horizontal)\nAns: "));
    while (Serial.available() == 0);
      Ans = Serial.readString();
      Serial.println(Ans);
    Serial.read();
    ...
    //Step 6
    Serial.print(F("Step 6: Read the value of force F using the load cell readings\nForce F = "));
    while (Serial.available() == 0);
      F = Serial.parseFloat();      //value = F
      Serial.print(F, 5);
      Serial.println(F("N"));
    Serial.read();
    ...
    //Step 9
    Serial.println(F("\nStep 9: Repeat steps 4 to 8 until you have records for at least 5 different values of d"));
    while (Serial.available() == 0);
    Serial.read();

    //Step 10
    Serial.println(F("\nStep 10: On a piece of graph paper, plot a graph of F (vertical axis) against 1/d (horizontal axis) and draw a line of best fit through your plotted points"));
    while (Serial.available() == 0);
    Serial.read();
    ...
    Serial.print(F("Actual weight = "));
    while (Serial.available() == 0);
      Aweight = Serial.parseFloat();
      Serial.println(Aweight, 5);
      Serial.println(F("N"));
    Serial.read();
  ...
  } while(PercentageError > 5 || PercentageError < -5);*/
